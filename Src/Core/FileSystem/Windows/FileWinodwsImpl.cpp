    ...
    namespace FileSystem {
        ...
        IFileImpl* IFileImpl::CreateRead(const char* file, bool allowWrite) {
            ...
            if (INVALID_HANDLE_VALUE == handle) {
                ...
                LOGW("create file faile, file:{}, error={}", file, error);
                return nullptr;
            }
            ...
            do {
                ...
                if (!ReadFile(fileHanle_, data, byteToRead, &bytesRead, &overlapped_)) {
                    ...
                    if (error != ERROR_IO_PENDING) {
                        LOGW("read file faile, file:{}, error={}", fileName_.c_str(), error);
                        return 0;
                    }

                    bytesRead = 0;
                    if (!GetOverlappedResult(fileHanle_, &overlapped_, &bytesRead, true)) {
                        LOGW("read file faile, file:{}, GetOverlappedResult failed", fileName_.c_str());
                        return 0;
                    }
                }
                ...
                if (byteToRead != bytesRead) {
                    LOGW("read file faile, file:{}, byteToRead({}) != bytesRead({})", fileName_.c_str(), byteToRead, bytesRead);
                    return 0;
                }
            } while (bytes > 0);
        ...
        int64_t FileWindowsImpl::Write(const char* data, int64_t bytes) {
            ...
            do {
                ...
                if (byteToWrite != bytesWrite) {
                    LOGW("write file faile, file:{}, byteToRead({}) != bytesRead({})", fileName_.c_str(), byteToWrite, bytesWrite);
                    return 0;
                }
            } while (bytes > 0);
        ...
        void FileWindowsImpl::UpdateSize() {
            ...
            if (!GetFileSizeEx(fileHanle_, &fileSize)) {
                DWORD error = GetLastError();
                LOGW("create file faile, file:{}, error={}", fileName_.c_str(), error);
                fileSize_ = -1;
            }
