...
#endif
    public:
        explicit CASQueue(const size_t capacity,
            ...
            static_assert(alignof(CASQueue<T>) == kCacheLineSize, "");
            static_assert(sizeof(CASQueue<T>) >= 3 * kCacheLineSize, "");
            assert(reinterpret_cast<char*>(&readIdx_) -
                reinterpret_cast<char*>(&writeIndex_) >=
        ...
        void emplace(Args &&...args) noexcept(
            ...
            auto const writeIdx = writeIndex_.load(std::memory_order_relaxed);
            auto nextWriteIdx = writeIdx + 1;
            if (nextWriteIdx == capacity_) {
                nextWriteIdx = 0;
            }
            while (nextWriteIdx == readIndexCache_) {
                readIndexCache_ = readIdx_.load(std::memory_order_acquire);
            }
        ...
        bool try_emplace(Args &&...args) noexcept(
            ...
            if (nextWriteIdx == readIndexCache_) {
                readIndexCache_ = readIdx_.load(std::memory_order_acquire);
                if (nextWriteIdx == readIndexCache_) {
                    return false;
                }
        ...
        T* front() noexcept {
            ...
            if (readIdx == writeIdxCache_) {
                writeIdxCache_ = writeIndex_.load(std::memory_order_acquire);
        ...
        void pop() noexcept {
            ...
            auto const readIdx = readIdx_.load(std::memory_order_relaxed);
            assert(writeIndex_.load(std::memory_order_acquire) != readIdx &&
