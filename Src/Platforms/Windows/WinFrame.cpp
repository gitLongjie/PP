...
#define UI_CLASSSTYLE_DIALOG     (CS_VREDRAW | CS_HREDRAW | CS_DBLCLKS | CS_SAVEBITS)
        ...
        LRESULT WinFrame::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
            ...
            const Core::Math::Rect& caption = context_.GetCaptionRect();
            ...
            if (pt.x >= rcClient.left + rcCaption.left && pt.x < rcClient.right - rcCaption.right \
                ...
                Core::Math::Point2d point(pt.x, pt.y);
                const Window::Control* control = context_.FindControl(point, Window::UIFIND_VISIBLE |
                    Window::UIFIND_HITTEST | Window::UIFIND_TOP_FIRST);
        ...
        LRESULT WinFrame::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
            const Core::Math::Size& size = context_.GetRoundCorner();
            if (!::IsIconic(*this)) {
                RECT rcWnd;
